...
## === ⭐  OTHERS =================================================
before-commit: tests qa-cs-fixer qa-phpstan qa-phpcpd qa-lint-yaml qa-lint-container qa-lint-schema ## Run before commit.
.PHONY: before-commit
...
setup-dev: ## set up dev.
	$(DOCKER_COMPOSE_APP) php bin/console ii:empty-database && $(DOCKER_COMPOSE_APP) php bin/console d:m:m -n && $(DOCKER_COMPOSE_UP) && $(DOCKER_COMPOSE_APP) php bin/console ii:l:ma && $(DOCKER_COMPOSE_APP) php bin/console ii:legacy:batch-souscription && $(DOCKER_COMPOSE_APP) php bin/console ii:config-user-dev
.PHONY: setup-dev
